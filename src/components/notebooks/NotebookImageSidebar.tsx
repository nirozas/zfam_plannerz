...
export const NotebookImageSidebar: React.FC<ImageSidebarProps> = ({
  selectedElement,
  onUpdateElement,
  onDuplicateElement,
  activePage,
    ...
    <div className="fixed right-8 top-1/2 -translate-y-1/2 w-[220px] bg-white/95 backdrop-blur-md border border-slate-200 shadow-[0_20px_50px_rgba(0,0,0,0.1)] z-[9999] rounded-[2rem] flex flex-col p-5 animate-in fade-in slide-in-from-right-12 duration-500 ease-out">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-900">Image Editor</h3>
        </div>
        <button 
          onClick={onClose}
        ...
        <section className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span className="text-[8px] font-black uppercase">Tilt</span>
            </button>
            <button 
          ...
          <div className="bg-slate-50 p-3 rounded-2xl flex items-center justify-between border border-slate-100">
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => onUpdateElement(selectedElement.id, { width: (selectedElement.width || 200) * 0.9, height: (selectedElement.height || 200) * 0.9 })} className="text-slate-400 hover:text-indigo-600"><MinusCircle size={16} /></button>
              <span className="text-[10px] font-black w-8 text-center">{Math.round(((selectedElement.width || 200) / 200) * 100)}%</span>
              <button onClick={() => onUpdateElement(selectedElement.id, { width: (selectedElement.width || 200) * 1.1, height: (selectedElement.height || 200) * 1.1 })} className="text-slate-400 hover:text-indigo-600"><PlusCircle size={16} /></button>
            </div>
          </div>
        </section>
