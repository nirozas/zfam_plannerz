        ...
        <div className="subtask-list">
            ...
            <div className="space-y-4">
                {subtasks.map(subtask => {
                    ...
                    return (
                        <div key={subtask.id} className="group flex flex-col gap-2 text-[13px] md:text-sm bg-white/30 p-2 md:p-3 rounded-xl transition-all hover:bg-white/60">
                            <div className="flex items-start gap-2 md:gap-3">
                                ...
                                >
                                    {subtask.isCompleted ? <CheckCircle size={isMobile ? 18 : 20} /> : <Circle size={isMobile ? 18 : 20} />}
                                </button>
                                ...
                                {!readOnly && (
                                    <div className={`flex items-center gap-1 transition-opacity ${isMobile ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                                        ...
                                        <button
                                            ...
                                            title="Due Date & Time"
                                        >
                                            <CalendarDays size={isMobile ? 12 : 14} />
                                        </button>
                                        <button
                                            type="button"
                                            onClick={() => {
                            ...
                            {editingImageUrlId === subtask.id && (
                                <div className="ml-6 md:ml-8 flex gap-2 animate-in slide-in-from-top-2 duration-200 p-2 bg-slate-50 rounded-lg border border-slate-100">
                                    <input
                                        type="text"
                                        value={urlInput}
                                    ...
                                    <button
                                        ...
                                        className="bg-indigo-600 text-white px-2 md:px-3 rounded-md text-[9px] md:text-[10px] font-black uppercase tracking-widest"
                                    >
                                        Apply
                                    </button>
                                    <button
                            ...
                            {editingTimingId === subtask.id && (
                                <div className="ml-6 md:ml-8 animate-in slide-in-from-top-2 duration-200 p-3 bg-indigo-50/50 rounded-xl border border-indigo-100 flex flex-col sm:flex-row gap-3">
                                    ...
                                    <button
                                        ...
                                        className="sm:mt-4 bg-indigo-600 text-white px-3 py-1.5 rounded-md text-[10px] font-black uppercase shadow-sm whitespace-nowrap"
                                    >
                                        Done
                                    </button>
                                </div>
                            ...
                            {editingTimingId !== subtask.id && (subtask.dueDate || subtask.dueTime) && (
                                <div className="ml-6 md:ml-8 flex items-center gap-3 text-[9px] md:text-[10px] font-bold text-indigo-400">
                            ...
                            {/* Subtask Image */}
                            {subtask.imageUrl && (
                                <div className="ml-6 md:ml-8 mt-1 relative group/img max-w-full">
                                    {isMobile ? (
                                        <div className="relative rounded-xl overflow-hidden border border-gray-100 shadow-sm">
                                            ...
                                            {!readOnly && (
                                                <button
                                                    ...
                                                    className="absolute top-2 right-2 bg-black/50 text-white rounded-full p-1.5 shadow-lg"
                                                >
                                                    <X size={12} />
                                                </button>
                                            )}
